...
export const ACTION_TYPES = {
  SEARCH_PHONES: 'phone/SEARCH_PHONES',
...
export default (state: PhoneState = initialState, action): PhoneState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_PHONES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_PHONES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_PHONES):
      return {
        ...state,
        loading: false,
...
const apiUrl = SERVER_API_URL + '/api/phones';
const apiSearchUrl = SERVER_API_URL + '/api/_search/phones';

// Actions

export const getSearchEntities: ICrudSearchAction<IPhone> = query => ({
  type: ACTION_TYPES.SEARCH_PHONES,
  payload: axios.get<IPhone>(`${apiSearchUrl}?query=` + query)
});
